...
CREATE POLICY "Users can view profiles in same school" ON users
  FOR SELECT USING (
    school_id IN (SELECT school_id FROM users WHERE id = auth.uid())
  );

-- Blurs policies
CREATE POLICY "Users can view sent blurs" ON blurs
  FOR SELECT USING (auth.uid() = sender_id);

CREATE POLICY "Users can view received blurs" ON blurs
  FOR SELECT USING (auth.uid() = receiver_id);

CREATE POLICY "Users can create blurs" ON blurs
  FOR INSERT WITH CHECK (auth.uid() = sender_id);

CREATE POLICY "Users can update received blurs" ON blurs
  FOR UPDATE USING (auth.uid() = receiver_id);

-- Polls policies
CREATE POLICY "Users can view school polls" ON polls
  FOR SELECT USING (
    school_id IN (SELECT school_id FROM users WHERE id = auth.uid())
  );
...
CREATE POLICY "Users can view votes they received" ON poll_votes
  FOR SELECT USING (auth.uid() = voted_for_id);
...
CREATE POLICY "Users can view school leaderboard" ON leaderboard
  FOR SELECT USING (
    school_id IN (SELECT school_id FROM users WHERE id = auth.uid())
  );
...
-- Trigger on blurs creation
CREATE TRIGGER trigger_update_last_active_on_blur
AFTER INSERT ON blurs
FOR EACH ROW
EXECUTE FUNCTION update_user_last_active();

-- Function to create notification on new blur
CREATE OR REPLACE FUNCTION notify_on_new_blur()
RETURNS TRIGGER AS $$
BEGIN
  ...
  VALUES (
    ...
    'Tap to unlock and see who it is',
    jsonb_build_object('blur_id', NEW.id)
  );
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
AFTER INSERT ON blurs
FOR EACH ROW
EXECUTE FUNCTION notify_on_new_blur();

-- Function to update school member count
CREATE OR REPLACE FUNCTION update_school_member_count()
RETURNS TRIGGER AS $$
BEGIN
...
FOR EACH ROW
EXECUTE FUNCTION update_school_member_count();

-- Function to initialize leaderboard entry for new users
CREATE OR REPLACE FUNCTION init_leaderboard_entry()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO leaderboard (user_id, school_id)
...
FOR EACH ROW
EXECUTE FUNCTION init_leaderboard_entry();

-- ============================================
-- SEED DATA (Demo schools)
-- ============================================
...
-- Create storage bucket for blur images
INSERT INTO storage.buckets (id, name, public) 
VALUES ('blurs', 'blurs', false);

-- Storage policies for blur images
CREATE POLICY "Users can upload own blurs"
...
CREATE POLICY "Users can view blurs they sent/received"
...
USING (
  ...
  (
    ...
    EXISTS (
      SELECT 1 FROM blurs
      WHERE blurs.image_url LIKE '%' || storage.objects.name || '%'
      AND (blurs.sender_id = auth.uid() OR blurs.receiver_id = auth.uid())
    )
  )
);
